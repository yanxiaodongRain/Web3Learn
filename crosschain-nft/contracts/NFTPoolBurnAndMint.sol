...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    function burnAndSendNFT(
        ...
        wnft.transferFrom(msg.sender, address(this), tokenId); //锁定到当前合约池子里

        wnft.burn(tokenId); //销毁NFT

        bytes memory payload = abi.encode(tokenId, newOwner); //编码tokenId和newOwner
        messageId = sendMessagePayLINK(chainSelector, receiver, payload); //发送跨链消息
        return messageId;
    }
    ...
    function _ccipReceive(
        ...
        // address neOwner, uinit256 tokenId
