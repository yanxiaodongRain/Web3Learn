...
/**
 Initializable： 用于升级使用
 UUPSUpgradeable： UUPS升级
 PausableUpgradeable： 提供 紧急停止/暂停功能，可以在合约中添加 whenNotPaused 或 whenPaused 修饰器，控制函数是否可用，好像这个合约里没有使用
 AccessControlUpgradeable: 提供 角色权限管理（RBAC）,可以定义多个角色，每个角色控制不同功能
 */
...
{
    using SafeERC20 for IERC20; //可以安全的使用SafeERC20的方法
    using Address for address;
    using Math for uint256;

    // ************************************** INVARIANT **************************************

    bytes32 public constant ADMIN_ROLE = keccak256("admin_role"); //配合AccessControlUpgradeable库使用的，管理员角色，可以设置重要参数
    bytes32 public constant UPGRADE_ROLE = keccak256("upgrade_role"); //配合AccessControlUpgradeable库使用的，升级角色，可以升级合约

    uint256 public constant ETH_PID = 0; //约定第 0 个池子是 ETH 池子，其余是 ERC20 Token 池子
    ...
    */
    ...
    struct Pool {
        ...
        address stTokenAddress; //质押的代币的地址，ETH = address(0x0)
        // Weight of pool
        uint256 poolWeight; // 池子权重, 用于奖励分配比例
        ...
        uint256 accMetaNodePerST; // 每个质押代币累计的奖励 (放大 1e18)
        // Staking token amount
        uint256 stTokenAmount; //当前池子总共质押的token
        // Min staking amount
        uint256 minDepositAmount; // 最小质押量
        // Withdraw locked blocks
        uint256 unstakeLockedBlocks; // 提现需要等待的区块数
    }

    //提现请求
    struct UnstakeRequest {
        // Request withdraw amount
        uint256 amount; // 申请提现的数量
        // The blocks when the request withdraw amount can be released
        uint256 unlockBlocks; // 区块高度到达才可提现
    }

    //用户信息
    struct User {
        // Staking token amount that user provided
        uint256 stAmount; // 用户当前质押量
        // Finished distributed MetaNodes to user
        uint256 finishedMetaNode; // 已领取或结算的奖励
        // Pending to claim MetaNodes
        uint256 pendingMetaNode; // 当前可领取奖励
        // Withdraw request list
        UnstakeRequest[] requests; // 用户提现请求
    }
    ...
    uint256 public MetaNodePerBlock; // 每块奖励数量
    ...
    uint256 public totalPoolWeight; // 总池子权重
    Pool[] public pool; // 所有池子
